#!/usr/bin/env python
# coding: utf-8
# Note each als project site has a EPSG number. Loop through every
# ...
# build a is2 simulation tool 
# input is2 points, option for 100m [ default 20m], reigon, project_name, delete las file [default yes]
# input for client set 
# 
...
def getCmdArgs():
        ...
        p.add_argument("-k", "--keep", dest="keep", required=False, action='store_true', help="keep converted laz file")
...
IS2_20M_FILE = '/gpfs/data1/vclgp/xiongl/ProjectIS2CalVal/result/is2_20m_misha_11302023.parquet'
...
def get_is2_projected_in_laz(gdf_is2, laz_path):
        print('# processing laz file', laz_path)
        name = laz_path.split('/')[-3]
        region = laz_path.split('/')[-4]
    #   ['region', 'name', 'area_ha', 'epsg', 'geometry']
        gdf_laz = gpd.read_parquet(f'../result/out_bounds/{region}_{name}_{os.path.basename(laz_path)[:-4]}.parquet')
    #   clip by gdf_laz
        is2_laz = gpd.clip(gdf_is2, gdf_laz)
    #   convert rest points using EPSG.
        is2_laz = is2_laz.to_crs(gdf_laz.crs)
        # Extract x and y coordinates into separate columns
        is2_laz['e'] = is2_laz['geometry'].x
        is2_laz['n'] = is2_laz['geometry'].y
    #   add orientation and output.
        is2_in_laz_projected_add_ori = add_orientation(is2_laz) # maybe add at beginning.
        res_out = RES_PATH + '/' + region + '/' + als_name 
        os.makedirs( res_out , exist_ok = True)
        is2_in_laz_projected_add_ori.to_parquet(f'{res_out}/is2_laz_{region}_{name}_{os.path.basename(laz_path)[:-4]}.parquet')
...
def sim_is2_laz(laz_path):
                    ...
                    if not os.path.isfile(out_wave): 
                        return None # edge case not exist.  find two files, has footprint, but no sim results.
...
if __name__ == '__main__':
            ...
            for index, row in gdf_als.iterrows(): 
                    ...
                    ##############################################################
                    # let us assume each project has only one epsg. it is right.
