...
def get_name(spatial_extent):
    ...
    if min_lat < 0: 
       L = 'S'
       min_lat = -min_lat
    name = L + str(min_lon) + N + str(min_lat)
    return name

@dask.delayed
def get_atl03_photon(spatial_extent):
...
@dask.delayed
def check_atl03_granules(spatial_extent):
    name = get_name(spatial_extent)
    short_name = 'ATL03'
    date_range = ['2018-01-01','2023-12-31']
    try: 
...
if __name__ == "__main__":
    ...
    intersection_cells.to_parquet('../result/atl03_grid_cells.parquet') # save grids file .
    print('-- number of 0.1 degree cells: ', len(intersection_cells))
    # loop through every cell 
    # get name
    #als_names = gdf['name'].unique()
    #for project in als_names:
    # for index, row in intersection_cells.iterrows(): 
    #     #gdf_pro = gdf[gdf['name'] == project]
    #     #spatial_extent = list( gdf_pro.total_bounds)
    #     spatial_extent = np.array(row['geometry'].bounds)
    #     print('-- Bounding box:', spatial_extent)
    #     if args.download:
    #         get_atl03_photon(spatial_extent)
    #     if args.check:
    #         check_atl03_granules(spatial_extent)
    # dask version try 
    print('## start client')
    client = Client()
    ...
    if args.download:
            cmds = [get_atl03_photon(np.array(row['geometry'].bounds)) for index, row in intersection_cells.iterrows()]
            _ = dask.persist(*cmds)
            progress(_)
            print('') 
    if args.check:
            cmds = [check_atl03_granules(np.array(row['geometry'].bounds)) for index, row in intersection_cells.iterrows()]
            _ = dask.persist(*cmds)
            progress(_)
            print('') 
    client.close()
    ...
    sys.exit("-- DONE")
# example use
# python 3is2subset_grids_fail.py  --check --test
# python 3is2subset.py  --download