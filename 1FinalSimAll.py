...
total_laz_files = glob.glob('/gpfs/data1/vclgp/data/gedi/imported/*/*/LAZ_ground/*.laz')
print(len(total_laz_files)) # 358, 796 laz files.
...
#laz_path = '/gpfs/data1/vclgp/data/gedi/imported/usa/usda_or/LAZ_ground/or_10_101.laz'
    ...
    ###########################################################################
                    ...
                    for is2_index, is2_row in is2_laz.iterrows(): # loop every segment. 
                                fid = is2_index
                                segment_footprints_utm = get_footprint(is2_index, is2_row)
                                out_coor = '../wave/coordinates_'+ str(fid) + '.txt'####
                                is2_laz[['e', 'n']].to_csv(out_coor, sep=' ', header = False,  index = False) 
                                # Create a list of Polygon geometries from the DataFrame
                                ...
                                #print(laz_nearby)
                                # laz file = '/gpfs/data1/vclgp/data/gedi/imported/usa/neon_osbs2017/LAZ_ground'
                                ...
                                out_las = '../wave/lasfiles_'+ str(fid) + '.txt'
                                new_path.to_csv(out_las, sep=' ', header = False,  index = False) 
                                out_wave = '../wave/wave_'+ str(fid) + '.h5'
                                #print('# run gediRat simulation...')
...
def get_footprint(is2_index, is2_row):
                    lat_c = is2_row['land_segments/latitude_20m'] 
                    lon_c = is2_row['land_segments/longitude_20m'] 
                    slope = is2_row['slope']
                    e1, n1 , zone1, letter1 = utm.from_latlon(lat_c, lon_c)
...
for index, row in gdf_als[6:8].iterrows(): 
        ...
        nCPU = len(laz_files)
        if nCPU > 50 : 
           nCPU = 50  # number of cores to use  
        ...
        # Use a list comprehension to filter out files (excluding subfolders)
        #files = [item for item in items if os.path.isfile(os.path.join(data_path, item))]
        # Get the count of files
        #print(len(files), data_path)
