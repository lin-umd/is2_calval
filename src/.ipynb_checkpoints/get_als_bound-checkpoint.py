...
las file --> las bounds --> merge to each als site --> merge to a single file.
lin xiong
07/03/2024
'''
...
LAZ_bounds_path = "/gpfs/data1/vclgp/xiongl/ProjectIS2CalVal/data/laz_bounds"
bounds_project_path = "/gpfs/data1/vclgp/xiongl/ProjectIS2CalVal/data/bounds_project"
lasinfo_path = "/gpfs/data1/vclgp/xiongl/ProjectIS2CalValdata/lasinfo_txt"
...
def get_project_bound(project):
        ...
        file_paths = glob.glob(f'{LAZ_bounds_path}/{region}_{name}_*.parquet')
        ...
        result_gdf.to_parquet(f'{bounds_project_path}/{region}_{name}.parquet')
        return result_gdf   # 
...
def get_laz_bound(f):
    region = f.split('/')[7]
    name = f.split('/')[8]
    out_txt = f'{lasinfo_path}/{region}_{name}_{os.path.basename(f)[:-4]}.txt'  # Replace with the path to your text file
    ...
    laz_gdf.to_parquet(f'{LAZ_bounds_path}/{region}_{name}_{os.path.basename(f)[:-4]}.parquet')
    return laz_gdf
...
if __name__ == '__main__':
    ...
    if args.all:
        ...
        projects = glob.glob(f'{bounds_project_path}/*.parquet')
        all = []
