...
ALL_SITES = '/gpfs/data1/vclgp/xiongl/ProjectIS2CalVal/data/all_sites_20231218.parquet'
CELLS_CROSS = '/gpfs/data1/vclgp/xiongl/ProjectIS2CalVal/result/atl03_grid_cells_v2.parquet'
...
def get_name(spatial_extent):
    lon, lat = spatial_extent
    min_lon = abs(int(lon * 10))
    min_lat = abs(int(lat * 10))
    letter_lon = 'E' if lon >= 0 else 'W'
    letter_lat = 'N' if lat >= 0 else 'S'
    return f"{letter_lon}{min_lon:04d}{letter_lat}{min_lat:04d}"
...
if __name__ == "__main__":
    ...
    gdf = gpd.read_parquet(ALL_SITES)
    if os.path.exists(CELLS_CROSS):
        intersection_cells=gpd.read_parquet(CELLS_CROSS)
    else:
        ...
        intersection_cells.to_parquet(CELLS_CROSS) # save grids file .
